    ...
    for i in range(0, 10):
        ...
        for j in range(0, 10):
            ...
            if can_key_be_reached(key, (p, q), depth-1):
                next_shard = (max(p, q), min(p, q))
                ret_vals.append(next_shard)
    return ret_vals


def get_factors_start(key):
    num_digits = get_digits_in_num(key)
    num_digits_half = num_digits//2 - 1
    starting_conditions = []
    for i in range(1, 100):
        for j in range(0, 100):
            i1 = i * 10**(num_digits_half-1)
            j1 = j * 10**(num_digits_half-1)
    ...
    starting_conditions = list(filter(lambda x: can_key_be_reached(key, x, num_digits_half-1), starting_conditions))
    ...
    for s in starting_conditions:
        res = get_factors_rec(key, s, num_digits_half-2)
        if res is not None:
            return res
    return None
...
def get_factors_start_mp(key, cores=2):
    ...
    starting_conditions = list(sorted(set(starting_conditions)))
    print(f"Starting conditions len: {len(starting_conditions)}: {starting_conditions}")
    args_list = [(key, i, num_digits_half-1) for i in starting_conditions]
...
def get_factors_start_mp_helper(args):
    key, shard, depth_pow = args
    return get_factors_rec(key, shard, depth_pow)
...
def get_factors_rec(key, shard, depth_pow):
    ...
    if depth_pow == 1:
        return None
    potentials = get_vals_meeting_squeeze_constraint(key, shard, depth_pow)
    potentials = sorted(set(potentials))
    for p in potentials:
        res = get_factors_rec(key, p, depth_pow - 1)
        if res is not None:
            return res
    return None
